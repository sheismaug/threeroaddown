...
function say(t, l, urgent) { try { if (!window.speechSynthesis || !t) return; if (urgent) window.speechSynthesis.cancel(); else if (window.speechSynthesis.speaking || window.speechSynthesis.pending) return; speak(t, l); } catch (e) {} }
...
export default function Nav3D({ route, problems, destName, onClose }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      map.on("load", () => {
        if (cancelled) return;
        try { map.setLight && map.setLight({ anchor: "viewport", color: "#ffffff", intensity: 0.5, position: [1.4, 200, 50] }); } catch (e) {}
        try { (map.getStyle().layers || []).forEach((l) => { if (/building/i.test(l.id)) map.removeLayer(l.id); }); } catch (e) {}
        try {
          map.addLayer({
            id: "nav3d-buildings", source: "openmaptiles", "source-layer": "building", type: "fill-extrusion", minzoom: 14,
            paint: {
              "fill-extrusion-color": ["interpolate", ["linear"], ["coalesce", ["get", "render_height"], 8], 0, "#dbe6f6", 12, "#c3d2ec", 40, "#a7bbdf", 120, "#8aa0cc"],
        ...
        ctx.current.timer = setInterval(() => {
          ...
          if (ctx.current.voiceOn) {
            if (alert && alert.dist <= 35 && !ctx.current.spoken.has("h" + alert.id)) {
              ...
              say(L === "en" ? `In ${mm} meters, ${TURN_EN[turnTHv] || "turn"}${mName ? " onto " + mName : ""}` : `ในอีก ${mm} เมตร ${instr}`, L, true);
            } else if ((mWp == null || distTurn > 90) && destLeft > 60 && d - ctx.current.straightD > 250) {
