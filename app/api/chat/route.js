...
export async function POST(req) {
  ...
  if (routes && routes.length) {
    toiletLines = routes.map((r) => {
      ...
      if (!list.length) return `เส้น ${r.index + 1}: ไม่พบห้องน้ำใกล้เส้นทาง`;
      const items = list.slice(0, 6).map((t) => `${t.name} (~กม.${(t.along / 1000).toFixed(2)} จากต้นทาง, ห่างเส้นทาง ~${t.off} ม.)`).join("; ");
      return `เส้น ${r.index + 1}: ${items}`;
    }).join("\n");
  }
  ...
  const system =
    ...
    "ถ้าผู้ใช้ถามเรื่อง 'ห้องน้ำ' ให้ตอบจากหัวข้อ [ห้องน้ำใกล้แต่ละเส้นทาง] เท่านั้น บอกชื่อ+ระยะจากต้นทาง (เมตร/กม.) ถ้าเส้นนั้นขึ้นว่า 'ไม่พบห้องน้ำใกล้เส้นทาง' ให้ตอบตรงๆ ว่าไม่พบห้องน้ำใกล้เส้นทางนี้ ห้ามเดาชื่อห้าง/สถานที่เอง " +
