...
export async function POST(req) {
  ...
  // กลางวัน/กลางคืน: ใช้ค่าจากเส้นทาง (คำนวณฝั่งแผนที่) ถ้าไม่มีก็เดาจากเวลาไทย
  const bkkHour = (new Date().getUTCHours() + 7) % 24;
  const night = routes && routes.length && typeof routes[0].night === "boolean"
    ? routes[0].night
    : !(bkkHour >= 7 && bkkHour < 18);

  let routeLines = "(ยังไม่มีข้อมูลเส้นทาง)";
  if (routes && routes.length) {
    routeLines = routes.map((r) => {
      // กลางวันไม่แสดง 'จุดมืด' (ไม่เกี่ยว), กลางคืนไม่แสดง 'ร่ม' (ไม่เกี่ยว) — ให้สอดคล้องกับการคิดคะแนน
      const parts = [`ปลอดภัย ${r.safe ?? "-"}`];
      if (!night) parts.push(`ร่ม ${r.shade ?? "-"}`);
      parts.push(`สวน ${r.green ?? "-"}`, `ห้องน้ำ ${r.toiletsNear ?? "-"}`, `กล้อง ${r.cameras ?? "-"}`);
      if (night) parts.push(`จุดมืด ${r.darkN ?? "-"}`);
      parts.push(`น้ำท่วม-ร้องเรียน(Traffy) ${r.floodN ?? "-"}`, `เสี่ยงน้ำท่วม-กทม. ${r.floodRiskN ?? "-"}`);
      return `เส้น ${r.index + 1}: ${(r.distance_m / 1000).toFixed(2)} กม. ~${r.duration_min} นาที | คะแนนรวม ${r.comfort ?? "-"} (${parts.join(", ")})${r.recommended ? " ⭐แนะนำ" : ""}`;
    }).join("\n");
  }
  ...
  const system = isToiletQ
    ...
    : ("คุณคือผู้ช่วยวางแผนการเดินเท้าย่านปทุมวัน ของแอป 'เดินกรุงเทพ' ตอบภาษาไทย " +
       (night
         ? "ขณะนี้เป็นช่วงกลางคืน: เรื่องความปลอดภัยให้ให้ความสำคัญกับแสงสว่าง/หลีกจุดมืด ห้ามพูดถึง 'ร่มเงา' (ไม่เกี่ยวตอนกลางคืน) "
         : "ขณะนี้เป็นช่วงกลางวัน: ห้ามพูดถึง 'จุดมืด/ความมืด' เป็นเหตุผลด้านความปลอดภัย (กลางวันไม่เกี่ยว) ให้เน้นร่มเงา กล้อง CCTV และจุดเสี่ยงทางเท้า/น้ำท่วมแทน ") +
       "เรื่องน้ำท่วมมี 2 แหล่งแยกกัน: 'น้ำท่วม-ร้องเรียน(Traffy)' (คำร้องจากประชาชน) กับ 'เสี่ยงน้ำท่วม-กทม.' (สำนักการระบายน้ำ) อย่าสับสนหรือเหมารวมเป็นอย่างเดียวกัน ระบุให้ตรงแหล่งเสมอ " +
       "สำคัญ: ถ้าปัจจัยใดมีค่าเท่ากันทุกเส้น (โดยเฉพาะเป็น 0 ทั้งหมด) ห้ามยกเป็นข้อได้เปรียบของเส้นใดเส้นหนึ่ง (ห้ามพูดว่า 'ต่ำสุด/น้อยสุด') ให้บอกแค่ว่าทุกเส้นเท่ากันหรือไม่พบ แล้วเลือกจุดเด่นจากปัจจัยที่ต่างกันจริงแทน " +
